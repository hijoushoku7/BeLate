...
import { AUTO_SETTLE_MS, LOCK_BEFORE_MS, NOTIFY_BEFORE_MS } from './domain';
...
export async function runCron(env: Env, now = Date.now()): Promise<void> {
  ...
  const starts = (await env.DB.prepare("SELECT * FROM events WHERE state='locked' AND meet_at-?<=?").bind(NOTIFY_BEFORE_MS, now).all<EventRow>()).results;
