...
export async function runCron(env: Env, now = Date.now()): Promise<void> {
  await env.DB.prepare("UPDATE events SET state='locked' WHERE state='open' AND meet_at-?<=?").bind(LOCK_BEFORE_MS, now).run();
  ...
  for (const event of starts) {
    ...
    for (const p of people.filter(p => p.status === 'joining' && p.arrived_at == null)) {
      ...
      if (reserved.meta.changes) await budgetedPush(env, p.user_id, [text(`集合時刻を過ぎました。現在地・到着を報告してください。\nhttps://liff.line.me/${env.LIFF_ID}?e=${event.id}&mode=report`)], 'user').catch(console.error);
    }
  }
  ...
  for (const event of expired) {
    ...
    await env.DB.prepare('INSERT INTO pending_group_notifications(group_id,message,created_at) VALUES(?,?,?)')
      .bind(event.group_id, JSON.stringify(text(settlementText(result))), now).run();
  }
}
