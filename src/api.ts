...
import { ARRIVAL_RADIUS_M, giftCount, GIFT_URL, calculateFine, distanceMeters, formatJst, lateMinutes, LOCK_BEFORE_MS, validFine } from './domain';
...
api.get('/settlement/:id', async c => {
  ...
  return c.json({
    ...
    gifts: resolved.gifts.map(d => ({ from: resolved.names.get(d.from) ?? d.from, to: resolved.names.get(d.to) ?? d.to, label: giftCount(d.amount) })),
    giftUrl: GIFT_URL,
  });
});
